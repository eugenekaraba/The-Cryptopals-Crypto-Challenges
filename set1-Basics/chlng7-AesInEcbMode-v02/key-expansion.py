...
def key_expansion(init_key_str):
    print "############################################################################################################"
    print "Key expansion started"
    print "The initial key in ascii characters format: ",init_key_str
    ...
    print "The initial key in ascii hex codes format: ",init_key
    keys = list()
    keys.append(init_key)
    for round in range(0, 10):
        print "The previous round key is in ascii hex codes format: ", keys[round]
        fourth_word_of_previous_key = keys[round][-8:]
        print "The 4th word of the previous round key is (hex integers): ",fourth_word_of_previous_key
        rotated_fourth_word_of_previous_key = rotate(fourth_word_of_previous_key, 2)
        print "The rotated 4th word of the previous round key is (hex integers):", rotated_fourth_word_of_previous_key
        ...
        print "The substituted rotated 4th word of the previous round key is (base 10 integers): ", \
            rotated_fourth_word_of_previous_key_substitution
        first_word_of_previous_key = keys[round][:8]
        print "The 1st word of the previous round key is (hex integers): ", first_word_of_previous_key
        ...
        print "The",round, "column of the Rcon table is (base 10 integers): ", roundth_column_of_rcon

        buffer_round_key = list()
        for i in range(4):
            print "debug: rotated_fourth_word_of_previous_key_substitution[i]", rotated_fourth_word_of_previous_key_substitution[i]
            print "debug: int(list(chunkstring(first_word_of_previous_key,2))[i], 16)", int(list(chunkstring(first_word_of_previous_key,2))[i], 16)
            print "debug: roundth_column_of_rcon[i])[2:].zfill(2))", roundth_column_of_rcon[i]
        ...
        print "The first word of the current round key is (base 10 integers): ", buffer_round_key
        ...
        for word in range(1,4):
            ...
            for i in range(4):
                buffer_round_key.append(hex(int(previous_word_of_current_round_key[i], 16) ^
                                            int(list(chunkstring(keys[round][(word * 8):(word * 8) + 8],2))[i], 16))[2:].zfill(2))
            # print "The ", word, " word of the previous round key is (hex integers): ", columnth_column_of_previous_key
            print "The  current buffer round key situation is (hex integers): ", buffer_round_key


        print "The final round key is (hex integers): ",buffer_round_key
        print "Round", round, "key:", ''.join(char for char in buffer_round_key)
        keys.append(''.join(char for char in buffer_round_key))
    print "Key expansion ended"
    for key in keys:
        print key
    print "############################################################################################################"
    return keys
